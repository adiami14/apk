...
/* loaded from: ItsOnFire.jar:androidx/compose/animation/EnterTransition.class */
public abstract class EnterTransition {
    ...
    /* loaded from: ItsOnFire.jar:androidx/compose/animation/EnterTransition$Companion.class */
    ...
    public final EnterTransition plus(@NotNull EnterTransition enter) {
        ...
        Fade fade2 = fade;
        if (fade == null) {
            fade2 = enter.getData$animation_release().getFade();
        }
        Slide slide = getData$animation_release().getSlide();
        Slide slide2 = slide;
        if (slide == null) {
            slide2 = enter.getData$animation_release().getSlide();
        }
        ChangeSize changeSize = getData$animation_release().getChangeSize();
        ChangeSize changeSize2 = changeSize;
        if (changeSize == null) {
            changeSize2 = enter.getData$animation_release().getChangeSize();
        }
        Scale scale = getData$animation_release().getScale();
        Scale scale2 = scale;
        if (scale == null) {
            scale2 = enter.getData$animation_release().getScale();
        }
        return new EnterTransitionImpl(new TransitionData(fade2, slide2, changeSize2, scale2));
    }
}
