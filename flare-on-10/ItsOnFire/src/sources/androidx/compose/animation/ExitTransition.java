...
/* loaded from: ItsOnFire.jar:androidx/compose/animation/ExitTransition.class */
public abstract class ExitTransition {
    ...
    /* loaded from: ItsOnFire.jar:androidx/compose/animation/ExitTransition$Companion.class */
    ...
    public final ExitTransition plus(@NotNull ExitTransition exit) {
        ...
        Fade fade2 = fade;
        if (fade == null) {
            fade2 = exit.getData$animation_release().getFade();
        }
        Slide slide = getData$animation_release().getSlide();
        Slide slide2 = slide;
        if (slide == null) {
            slide2 = exit.getData$animation_release().getSlide();
        }
        ChangeSize changeSize = getData$animation_release().getChangeSize();
        ChangeSize changeSize2 = changeSize;
        if (changeSize == null) {
            changeSize2 = exit.getData$animation_release().getChangeSize();
        }
        Scale scale = getData$animation_release().getScale();
        Scale scale2 = scale;
        if (scale == null) {
            scale2 = exit.getData$animation_release().getScale();
        }
        return new ExitTransitionImpl(new TransitionData(fade2, slide2, changeSize2, scale2));
    }
}
