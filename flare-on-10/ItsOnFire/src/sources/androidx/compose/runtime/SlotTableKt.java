...
/* loaded from: ItsOnFire.jar:androidx/compose/runtime/SlotTableKt.class */
public final class SlotTableKt {
    ...
    public static final int auxIndex(int[] iArr, int i2) {
        ...
        if (i3 >= iArr.length) {
            ...
            countOneBits = countOneBits(iArr[i3 + 1] >> 29) + iArr[i3 + 4];
        }
        return countOneBits;
    }
    ...
    public static final boolean hasObjectKey(int[] iArr, int i2) {
        boolean z = true;
        if ((iArr[(i2 * 5) + 1] & ObjectKey_Mask) == 0) {
            z = false;
        }
        return z;
    }
    ...
    public static final int search(ArrayList<Anchor> arrayList, int i2, int i3) {
        ...
        while (i4 <= size) {
            ...
            int i6 = location$runtime_release;
            if (location$runtime_release < 0) {
                i6 = location$runtime_release + i3;
            }
            int compare = Intrinsics.compare(i6, i2);
            if (compare < 0) {
                i4 = i5 + 1;
            } else if (compare <= 0) {
                return i5;
