...
/* loaded from: ItsOnFire.jar:androidx/compose/runtime/snapshots/SnapshotStateMap.class */
public final class SnapshotStateMap<K, V> implements Map<K, V>, StateObject, KMutableMap {
    ...
    /* loaded from: ItsOnFire.jar:androidx/compose/runtime/snapshots/SnapshotStateMap$StateMapStateRecord.class */
    ...
    private final void update(Function1<? super PersistentMap<K, ? extends V>, ? extends PersistentMap<K, ? extends V>> function1) {
        ...
        StateMapStateRecord stateMapStateRecord2 = (StateMapStateRecord) SnapshotKt.current(stateMapStateRecord, companion.getCurrent());
        PersistentMap<K, V> persistentMap = (PersistentMap) ((PersistentMap<K, ? extends V>) function1.invoke(stateMapStateRecord2.getMap$runtime_release()));
        if (persistentMap != stateMapStateRecord2.getMap$runtime_release()) {
            obj = SnapshotStateMapKt.sync;
            synchronized (obj) {
                try {
                    StateMapStateRecord stateMapStateRecord3 = (StateMapStateRecord) getFirstStateRecord();
                    ...
                    synchronized (SnapshotKt.getLock()) {
                        current = companion.getCurrent();
                        StateMapStateRecord stateMapStateRecord4 = (StateMapStateRecord) SnapshotKt.writableRecord(stateMapStateRecord3, this, current);
                        stateMapStateRecord4.setMap$runtime_release(persistentMap);
                        stateMapStateRecord4.setModification$runtime_release(stateMapStateRecord4.getModification$runtime_release() + 1);
                        InlineMarker.finallyStart(1);
                    }
                    InlineMarker.finallyEnd(1);
