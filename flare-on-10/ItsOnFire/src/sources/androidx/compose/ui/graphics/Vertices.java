...
/* loaded from: ItsOnFire.jar:androidx/compose/ui/graphics/Vertices.class */
public final class Vertices {
    ...
    private Vertices(int i2, final List<Offset> list, List<Offset> list2, List<Color> list3, List<Integer> list4) {
        ...
        while (true) {
            if (i3 >= size) {
                z = false;
                break;
            } else if (function1.invoke(list4.get(i3)).booleanValue()) {
                z = true;
                break;
            } else {
                i3++;
    ...
    private final int[] encodeColorList(List<Color> list) {
        ...
        for (int i2 = 0; i2 < size; i2++) {
            iArr[i2] = ColorKt.m2470toArgb8_81llA(list.get(i2).m2424unboximpl());
        }
        return iArr;
    }
    ...
    private final float[] encodePointList(List<Offset> list) {
        ...
        for (int i2 = 0; i2 < size; i2++) {
            long m2177unboximpl = list.get(i2 / 2).m2177unboximpl();
            fArr[i2] = i2 % 2 == 0 ? Offset.m2167getXimpl(m2177unboximpl) : Offset.m2168getYimpl(m2177unboximpl);
        }
        return fArr;
    }
    ...
    public final int m2765getVertexModec2xauaI() {
