...
/* loaded from: ItsOnFire.jar:androidx/compose/ui/graphics/colorspace/ColorSpaceKt.class */
public final class ColorSpaceKt {
    ...
    public static final ColorSpace adapt(@NotNull ColorSpace colorSpace, @NotNull WhitePoint whitePoint, @NotNull Adaptation adaptation) {
        ...
        if (ColorModel.m2773equalsimpl0(colorSpace.m2783getModelxdoWZVw(), ColorModel.Companion.m2780getRgbxdoWZVw())) {
    ...
    public static final boolean compare(@NotNull WhitePoint a2, @NotNull WhitePoint b2) {
        ...
        if (Math.abs(a2.getX() - b2.getX()) >= 0.001f || Math.abs(a2.getY() - b2.getY()) >= 0.001f) {
            z = false;
        }
        return z;
    }
    ...
    public static final Connector m2784connectYBCOT_4(@NotNull ColorSpace connect, @NotNull ColorSpace destination, int i2) {
        Intrinsics.checkNotNullParameter(connect, "$this$connect");
        Intrinsics.checkNotNullParameter(destination, "destination");
        if (connect == destination) {
            return Connector.Companion.identity$ui_graphics_release(connect);
        }
        long m2783getModelxdoWZVw = connect.m2783getModelxdoWZVw();
        ColorModel.Companion companion = ColorModel.Companion;
        return (ColorModel.m2773equalsimpl0(m2783getModelxdoWZVw, companion.m2780getRgbxdoWZVw()) && ColorModel.m2773equalsimpl0(destination.m2783getModelxdoWZVw(), companion.m2780getRgbxdoWZVw())) ? new Connector.RgbConnector((Rgb) connect, (Rgb) destination, i2, null) : new Connector(connect, destination, i2, null);
    }

    /* renamed from: connect-YBCOT_4$default */
    public static /* synthetic */ Connector m2785connectYBCOT_4$default(ColorSpace colorSpace, ColorSpace colorSpace2, int i2, int i3, Object obj) {
        if ((i3 & 1) != 0) {
            colorSpace2 = ColorSpaces.INSTANCE.getSrgb();
        }
        if ((i3 & 2) != 0) {
            i2 = RenderIntent.Companion.m2806getPerceptualuksYyKA();
        }
        return m2784connectYBCOT_4(colorSpace, colorSpace2, i2);
    }
    ...
    public static final float[] mul3x3(@NotNull float[] lhs, @NotNull float[] rhs) {
        ...
        float f27 = rhs[8];
        float f28 = rhs[6];
        return new float[]{(f2 * f3) + (f4 * f5) + (f6 * f7), (f8 * f9) + (f5 * f10) + (f11 * f7), (f12 * f9) + (f14 * f13) + (f7 * f15), (f17 * f16) + (f4 * f18) + (f6 * f19), (f20 * f21) + (f10 * f18) + (f11 * f19), (f21 * f22) + (f13 * f23) + (f19 * f15), (f16 * f24) + (f25 * f26) + (f6 * f27), (f20 * f28) + (lhs[4] * f26) + (f11 * f27), (f22 * f28) + (lhs[5] * rhs[7]) + (f15 * f27)};
    }
    ...
    public static final float[] mul3x3Diag(@NotNull float[] lhs, @NotNull float[] rhs) {
        ...
        float f11 = rhs[4];
        float f12 = lhs[2];
        return new float[]{f2 * f3, f4 * f5, f6 * f7, f9 * f8, f11 * f10, rhs[5] * f12, f8 * rhs[6], f10 * rhs[7], f12 * rhs[8]};
    }
