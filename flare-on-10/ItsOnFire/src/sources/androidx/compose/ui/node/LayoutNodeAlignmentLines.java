...
/* loaded from: ItsOnFire.jar:androidx/compose/ui/node/LayoutNodeAlignmentLines.class */
public final class LayoutNodeAlignmentLines {
    ...
    private static final void recalculate$addAlignmentLine(LayoutNodeAlignmentLines layoutNodeAlignmentLines, AlignmentLine alignmentLine, int i2, LayoutNodeWrapper layoutNodeWrapper) {
        ...
        loop0: while (true) {
            ...
            do {
                Offset = layoutNodeWrapper.m4082toParentPositionMKHz9U(Offset);
                wrappedBy$ui_release = layoutNodeWrapper.getWrappedBy$ui_release();
                Intrinsics.checkNotNull(wrappedBy$ui_release);
                if (Intrinsics.areEqual(wrappedBy$ui_release, layoutNodeAlignmentLines.layoutNode.getInnerLayoutNodeWrapper$ui_release())) {
                    break loop0;
                }
                layoutNodeWrapper = wrappedBy$ui_release;
            } while (!wrappedBy$ui_release.getProvidedAlignmentLines().contains(alignmentLine));
            i2 = wrappedBy$ui_release.get(alignmentLine);
            layoutNodeWrapper = wrappedBy$ui_release;
        }
        roundToInt = MathKt__MathJVMKt.roundToInt(alignmentLine instanceof HorizontalAlignmentLine ? Offset.m2168getYimpl(Offset) : Offset.m2167getXimpl(Offset));
        ...
        if (map.containsKey(alignmentLine)) {
            ...
            i3 = AlignmentLineKt.merge(alignmentLine, ((Number) value).intValue(), roundToInt);
        }
        map.put(alignmentLine, Integer.valueOf(i3));
    }
    ...
    public final void recalculate() {
        ...
        do {
            ...
            if (layoutNode.isPlaced()) {
                ...
                while (true) {
                    LayoutNodeWrapper wrappedBy$ui_release = innerLayoutNodeWrapper$ui_release.getWrappedBy$ui_release();
                    Intrinsics.checkNotNull(wrappedBy$ui_release);
                    if (Intrinsics.areEqual(wrappedBy$ui_release, this.layoutNode.getInnerLayoutNodeWrapper$ui_release())) {
                        break;
                    }
                    Iterator<T> it = wrappedBy$ui_release.getProvidedAlignmentLines().iterator();
                    while (true) {
                        innerLayoutNodeWrapper$ui_release = wrappedBy$ui_release;
                        if (it.hasNext()) {
