...
/* loaded from: ItsOnFire.jar:androidx/compose/ui/text/input/EditingBuffer.class */
public final class EditingBuffer {
    ...
    /* loaded from: ItsOnFire.jar:androidx/compose/ui/text/input/EditingBuffer$Companion.class */
    ...
    private EditingBuffer(AnnotatedString annotatedString, long j2) {
        ...
        this.selectionStart = TextRange.m4473getMinimpl(j2);
        this.selectionEnd = TextRange.m4472getMaximpl(j2);
        this.compositionStart = -1;
        this.compositionEnd = -1;
        int m4473getMinimpl = TextRange.m4473getMinimpl(j2);
        int m4472getMaximpl = TextRange.m4472getMaximpl(j2);
        if (m4473getMinimpl < 0 || m4473getMinimpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("start (" + m4473getMinimpl + ") offset is outside of text region " + annotatedString.length());
        } else if (m4472getMaximpl < 0 || m4472getMaximpl > annotatedString.length()) {
            throw new IndexOutOfBoundsException("end (" + m4472getMaximpl + ") offset is outside of text region " + annotatedString.length());
        } else if (m4473getMinimpl <= m4472getMaximpl) {
        } else {
            throw new IllegalArgumentException("Do not set reversed range: " + m4473getMinimpl + " > " + m4472getMaximpl);
        }
    }
    ...
    public final void delete$ui_text_release(int i2, int i3) {
        ...
        long m4554updateRangeAfterDeletepWDy79M = EditingBufferKt.m4554updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.selectionStart, this.selectionEnd), TextRange);
        this.selectionStart = TextRange.m4473getMinimpl(m4554updateRangeAfterDeletepWDy79M);
        this.selectionEnd = TextRange.m4472getMaximpl(m4554updateRangeAfterDeletepWDy79M);
        if (hasComposition$ui_text_release()) {
            long m4554updateRangeAfterDeletepWDy79M2 = EditingBufferKt.m4554updateRangeAfterDeletepWDy79M(TextRangeKt.TextRange(this.compositionStart, this.compositionEnd), TextRange);
            if (TextRange.m4469getCollapsedimpl(m4554updateRangeAfterDeletepWDy79M2)) {
                commitComposition$ui_text_release();
                return;
            }
            this.compositionStart = TextRange.m4473getMinimpl(m4554updateRangeAfterDeletepWDy79M2);
            this.compositionEnd = TextRange.m4472getMaximpl(m4554updateRangeAfterDeletepWDy79M2);
        }
    }
