...
/* loaded from: ItsOnFire.jar:androidx/core/content/res/CamUtils.class */
final class CamUtils {
    ...
    public static float[] xyzFromInt(int i2) {
        ...
        float f7 = fArr3[2];
        float[] fArr4 = fArr[2];
        return new float[]{(f2 * linearized) + (f3 * linearized2) + (f4 * linearized3), (f5 * linearized) + (f6 * linearized2) + (f7 * linearized3), (linearized * fArr4[0]) + (linearized2 * fArr4[1]) + (linearized3 * fArr4[2])};
    }
