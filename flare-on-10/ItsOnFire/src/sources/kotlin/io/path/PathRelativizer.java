...
/* loaded from: ItsOnFire.jar:kotlin/io/path/PathRelativizer.class */
    ...
    public final Path tryRelativeTo(@NotNull Path path, @NotNull Path base) {
        ...
        for (int i2 = 0; i2 < min; i2++) {
            ...
            if (!Intrinsics.areEqual(name, path2)) {
                break;
            } else if (!Intrinsics.areEqual(normalize2.getName(i2), path2)) {
        ...
        if (Intrinsics.areEqual(normalize2, normalize) || !Intrinsics.areEqual(normalize, emptyPath)) {
            ...
            if (endsWith$default) {
                ...
                relativize = fileSystem.getPath(dropLast, new String[0]);
            }
        } else {
            relativize = normalize2;
        }
        Path r2 = relativize;
