...
/* loaded from: ItsOnFire.jar:kotlin/math/MathKt__MathJVMKt.class */
public class MathKt__MathJVMKt extends MathKt__MathHKt {
    ...
    public static final double acosh(double d2) {
        ...
        if (d2 < 1.0d) {
            ...
            sqrt = Math.log(d2) + Constants.LN2;
        } else {
            double d3 = 1;
            double d4 = d2 - d3;
            if (d4 >= Constants.taylor_n_bound) {
                sqrt = Math.log(d2 + Math.sqrt((d2 * d2) - d3));
            } else {
                double sqrt2 = Math.sqrt(d4);
                double d5 = sqrt2;
                if (sqrt2 >= Constants.taylor_2_bound) {
                    d5 = sqrt2 - (((sqrt2 * sqrt2) * sqrt2) / 12);
                }
    ...
    public static int roundToInt(double d2) {
        ...
        return d2 > 2.147483647E9d ? Integer.MAX_VALUE : d2 < -2.147483648E9d ? Integer.MIN_VALUE : (int) Math.round(d2);
    }
    ...
    public static final double truncate(double d2) {
        double d3 = d2;
        if (!Double.isNaN(d2)) {
            d3 = Double.isInfinite(d2) ? d2 : d2 > 0.0d ? Math.floor(d2) : Math.ceil(d2);
        }
        return d3;
    }
